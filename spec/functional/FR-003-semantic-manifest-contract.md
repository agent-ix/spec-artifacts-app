...
- The FR-035 module-manifest schema, bundled for the suite at
  ...
  the reference-form `data_schema` of filament-core-service FR-035 CR-003.
...
- `semantic.imports` SHALL map each imported module reference
  (`^[a-z][a-z0-9-]*/[a-z][a-z0-9-]*$`) to the list of type names this module
  references from it.
- If a shipped skeleton, negative fixture, or `mappings.yaml` entry names an
  ...
  `type` is absent from that module's declared type list, then the module's test
  suite SHALL fail naming the module and the type.
- If `semantic.imports` names this module's own `package`, then the module's test
  suite SHALL fail naming the self-import, because a module importing itself is
  the degenerate one-node cycle.
- If the import graph built from this module's `semantic.imports` and the
  `semantic.imports` of every module installed under the Filament module root
  contains a cycle reaching this module, then the module's test suite SHALL fail
  naming every module on the cycle in traversal order.
- The suite SHALL report a missing import and a cycle as distinct diagnostics; a
  cycle SHALL NOT be reported as a missing import.
...
| FR-003-AC-4 | `semantic.imports` names `agent-ix/spec-artifacts-iso` with the type list this module references; every `ImportedTypeRef` appearing in a skeleton, a negative fixture, or `mappings.yaml` names a module and a type that `semantic.imports` declares; an undeclared module and an undeclared type each fail naming both values. | Test (TC-014) |
| FR-003-AC-5 | An import graph containing a cycle that reaches this module fails naming every module on the cycle in traversal order, and that failure is distinct from the missing-import failure of AC-4 — the fixture graph exercises a two-module cycle and a three-module cycle. | Test (TC-015) |
...
- **Downstream**: [NFR-001](../non-functional/NFR-001-reproducible-additive-projection.md), [IT-002](../integration/IT-002-module-load-and-extraction-roundtrip.md)
