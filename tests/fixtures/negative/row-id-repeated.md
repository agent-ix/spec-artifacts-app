...
expect: mapping.row-id-repeated
because: "a row id must be unique within its table"
---
<!-- Violates exactly one rule: `AS-900-CAP-2` is reused as the id of the third capability row. -->
...
| AS-900-CAP-2 | Hand off to fulfilment | Publish an accepted order to the fulfilment topic exactly once. | AS-900-ACT-4 |
