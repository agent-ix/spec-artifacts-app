...
because: "a capability row id must match the AS-900-CAP-N pattern"
---
<!-- Violates exactly one rule: one capability row is numbered `AS-900-XYZ-1`. -->
...
|---|---|---|---|
| AS-900-XYZ-1 | Submit an order | Accept an order from the storefront or the partner API and persist it as submitted. | AS-900-ACT-1, AS-900-ACT-3 |
