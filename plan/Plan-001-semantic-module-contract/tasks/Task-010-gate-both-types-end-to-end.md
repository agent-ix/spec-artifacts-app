...
- [x] **Skeleton to record to schema.** For `ApplicationSpec` and `MasterRequirements`: the shipped skeleton validates under `validate_document`, extracts under `extract_semantic` with zero error diagnostics, maps to a record under the reference mapping, and that record validates against the emitted schema the manifest binds by digest.
- [x] **Both `## Properties` forms.** The typed table and the `sysml` fence yield the same `FieldDecl` list, in the same order.
- [x] **Every negative fixture refused**, each by the check its `expect` names.
- [x] **No skipped row.** The gate fails if any semantic row skipped rather than ran.
- [x] **`make lint` and `make schemas-check` both green** on the committed tree, and `quire validate` structurally clean over `spec/**/*.md` and `plan/**/*.md`.
